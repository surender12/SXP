...
- (id)initWithFrame:(CGRect)frame valueDictionary:(NSDictionary *)valueDictionary
{
    ...
    if (self) {
        ...
        [[NSNotificationCenter defaultCenter] addObserver:self
                                                 selector:@selector(receiveSkillNotification:)
                                                     name:@"SkillTypeNotification"
                                                   object:nil];
...
- (void)receiveSkillNotification:(NSNotification *) notification
{
    NSDictionary* userInfo = notification.userInfo;
    _isClickable = [userInfo[@"isClickable"] intValue];
    if ([[notification name] isEqualToString:@"SkillTypeNotification"]){
        NSLog (@"Successfully received the SkillTypeNotification!");
     
//        for (UIButton * cat in self.subviews) {
//            if (cat.tag == 921) {
//                if (_isClickable == 1) {
...
- (void)drawLabelWithMaxLength:(CGFloat)maxLength labelArray:(NSArray *)labelArray angleArray:(NSArray *)angleArray
{
    ...
    for (NSString *labelString in labelArray) {
        ...
        label.titleLabel.font = FontOpenSans(11);
        label.titleLabel.textAlignment = NSTextAlignmentCenter;
        label.titleLabel.textColor = [UIColor whiteColor];
        [label setTitle:labelString forState:UIControlStateNormal];
        label.tag = labelTag;
        label.frame = CGRectMake(x - 5*fontSize/2, y - fontSize/2, 5*fontSize, fontSize);
...
//    NSLog(@"senderTag---%@",sender.titleLabel.text);
    
    self.blockFilter(sender.titleLabel.text);
